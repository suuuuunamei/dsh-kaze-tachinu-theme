...
<p align="center">
  ...
  <em>天蓝玻璃拟态 · 晴空电影壁纸 · 自绘矢量徽标 · 输入卡重绘 · 主题化占位文案 · 统一滚动条 · 一条命令安装</em>
</p>
...
输入卡重绘为云海蓝玻璃胶囊，占位文案按界面语言自动替换（皮肤中心 / 动态注入场景；静态 bundle 场景下中英文界面共用中文文案）。替换基于**前缀匹配**：即使宿主在默认提示后追加「… / 调用指令 @ 文件或对话」这类说明也能命中。
...
文案映射集中在 `bundle/client.js`、`plugin/client.js` 与 `skin/kaze-tachinu/hooks.mjs` 的占位规则表中，按需修改后重启生效。

### 调色

颜色集中在 `bundle/client.js` 与 `skin/kaze-tachinu/` 两处（token 覆盖与组件样式），改完重启 `dsh web` 并强刷。速查表见 [docs/theme-tokens.md](docs/theme-tokens.md)。
...
bundle/client.js     # 插件浏览器半区：token 覆盖 + 组件样式 + DOM 补丁 + 占位文案
...
A: 占位替换作用于宿主输入组件的 placeholder 属性（前缀匹配可命中「描述…/ 调用指令 @ 文件或对话」这类带后缀的默认文案）。个别桌面发行版若以自绘视觉层渲染占位文本而非真实 placeholder 属性，则该替换不适用（与 dsh-kimino-theme 在此类发行版上的限制相同）；标准 DSH Web 客户端下生效。
...
- 个别 DSH Desktop 发行版不执行插件浏览器半区（只加载宿主半区），此类环境下品牌替换与占位文案等 DOM 级效果受限——与 dsh-kimino-theme 在同类发行版的限制一致；标准 DSH Web 客户端可完整生效。
